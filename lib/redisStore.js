...
function RedisStore (options) {
  ...
  if (!options.host)
    throw new Error('a host is required to instantiate a redis store')
  if (!options.port)
    throw new Error('a port is required to instantiate a redis store')
...
RedisStore.prototype.trace = function trace (
  serviceName,
  queueName,
  type,
  cid,
